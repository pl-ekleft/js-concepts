...
setTimeout(second, 0) // Second - синхронный вызов, вызывается последним т.к. обрабатывается сторонним API, добавляется в очередь концепта event callback queue последним, далее попадает в call stack и после уже event loop его обработает
 ...
 *
 * Визуально ознакомиться можно здесь: http://latentflip.com/loupe/
**/
